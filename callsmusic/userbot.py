...
from InnexiaMusic.logging import LOGGER
        ...
        if config.STRING1:
            ...
            try:
                await self.one.send_message(
                    config.LOG_GROUP_ID, f"**Assistant One Started as {self.one.name}**"
                )
            except:
        ...
        if config.STRING2:
            ...
            try:
                await self.two.send_message(
                    config.LOG_GROUP_ID, f"**Assistant Two Started as {self.two.name}**"
                )
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING3:
            ...
            try:
                await self.three.send_message(
                    config.LOG_GROUP_ID, f"**Assistant Three Started as {self.three.name}**"
                )
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING4:
            ...
            try:
                await self.four.send_message(
                    config.LOG_GROUP_ID, f"**Assistant Four Started as {self.four.name}**"
                )
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING5:
            ...
            try:
                await self.five.send_message(
                    config.LOG_GROUP_ID, f"**Assistant Five Started as {self.five.name}**"
                )
            except:
                LOGGER(__name__).error(
