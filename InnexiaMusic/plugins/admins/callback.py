...
from InnexiaMusic.utils.formatters import seconds_to_min
from InnexiaMusic.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Music.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Music.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
